...
1. ダッシュボードを開く
2. 作成ウィザードでテーマと自動化モードを指定する
3. プロジェクト詳細で作成結果を確認する
4. 台本を編集して保存する
5. 素材をアップロードし、用途割当とサムネイル表示を確認する
6. タイムラインの再生範囲を調整して保存する
7. プレビュー情報を取得し、`preview.mp4` 再生要素と `final.mp4` ダウンロード導線を確認してレンダリングジョブを作成する
8. 設定画面でAPI接続診断と出力プリセット保存を確認する
