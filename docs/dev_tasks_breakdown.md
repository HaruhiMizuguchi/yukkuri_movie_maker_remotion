...
## 0. 前提整備
- [ ] `.env` と `config/` のテンプレ整理（必要キー一覧の明文化）
- [ ] Prisma `schema.prisma` の見直し（成果物管理の拡張余地確認）
- [ ] ローカル開発起動フローの整理（`pnpm dev` の役割確認）

## 1. データモデル / 共有型
- [ ] `packages/shared` に台本（Script）型定義を追加
- [ ] 生成物（音声/字幕/画像/動画）メタデータ型の追加
