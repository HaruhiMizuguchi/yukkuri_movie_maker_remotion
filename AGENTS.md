...
- `.gitignore` 追加と最新コミットからの生成物削除だけでは、未送信の過去コミットに含まれる100MB超MP4によるGitHubのGH001拒否は解消しない。元履歴をローカルブランチへ保存し、未送信履歴の生成物だけを除外してHEADツリー一致とfast-forward可否を検証すれば、コードやローカル動画を保持したまま通常pushできる。
