...
describe("generateBestAvailableVideo", () => {
  ...
  it("AivisSpeech 実接続で Remotion ショーケース動画を生成できる", async () => {
    ...
    const projectRoot = path.join(outputRoot, summary.runId, "projects", summary.projectId);
    ...
    const visualPlan = JSON.parse(await fs.readFile(visualPlanPath, "utf-8")) as {
    ...
    expect(composition.visualTrackCount).toBeGreaterThanOrEqual(composition.shotCount);
    ...
    expect(
      visualPlan.tracks.some(
        (track) => track.sourceType === "video" || track.sourceType === "image"
      )
    ).toBe(true);
